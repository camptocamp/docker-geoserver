...
def test_get_capabilities(connection):
    ...
    answer = connection.get_xml(url="ows?SERVICE=WMS&REQUEST=GetCapabilities&VERSION=1.3.0", cache_expected=CacheExpected.DONT_CARE, cors=False)
...
def test_get_map(connection):
    answer = connection.get_raw(url="ows?SERVICE=WMS&VERSION=1.3.0&REQUEST=GetMap&LAYERS=poi&STYLES=&CRS=EPSG:4326&BBOX=-180,-90,180,90&WIDTH=600&HEIGHT=300&FORMAT=image/png", cache_expected=CacheExpected.DONT_CARE, cors=False)
